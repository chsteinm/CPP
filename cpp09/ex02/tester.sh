...
for ((current_input_size=1; current_input_size<=total_input_sizes; current_input_size++)); do
	...
	for ((combination_index=1; combination_index<=combinations_per_input; combination_index++)); do
		...
		# Generate a random input list of 'current_input_size' numbers between min_random_value and max_random_value without duplicates
		if (( current_input_size > max_random_value - min_random_value + 1 )); then
			echo -e "${RED}Error: Not enough unique numbers in the range for input size $current_input_size. Exiting...${RESET}"
			exit 1
		fi
